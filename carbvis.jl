...
function fluxglobe(slider)
	...
	@lift(Label(fig[1, 1, Top()], "Fluxnet data availablity : $($num_site) sites"))

    pltobj = @lift(meshscatter!(ax, toPoints3D[$index];
    markersize = mag[$index]/1000,
    color = mag[$index],
	...
	@lift(Colorbar(fig[1,2], $pltobj, label="Number of Years", height = Relative(1/2)))
    fig
end
